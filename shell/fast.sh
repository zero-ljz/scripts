...
# if [ "$OSID" = "ubuntu" ]; then
# wget -O exa_0.9.0-4_amd64.deb -c ${base_url}http://old-releases.ubuntu.com/ubuntu/pool/universe/r/rust-exa/exa_0.9.0-4_amd64.deb
...
curl -fsSL ${base_url}https://download.docker.com/linux/${OSID}/gpg | gpg --dearmor -o /etc/apt/keyrings/docker.gpg
chmod a+r /etc/apt/keyrings/docker.gpg
echo -e "\n\n\n 设置存储库"
echo  "deb [arch="$(dpkg --print-architecture)" signed-by=/etc/apt/keyrings/docker.gpg] ${base_url}https://download.docker.com/linux/${OSID} "$(. /etc/os-release && echo "$VERSION_CODENAME")" stable" | tee /etc/apt/sources.list.d/docker.list > /dev/null
echo -e "\n\n\n 更新APT包索引"
apt-get update
...
wget -O /etc/aria2/aria2.conf ${base_url}https://1fxdpq.dm.files.1drv.com/y4mIiwJL9lNeIdO8lXxaVlJ8CgaezUd3kIe7r8ZcAFytG78pUdSN0RprxwsYBW87AwMyXDAtEc3mLeTYBWHf_D4ngSWtjlCGvsoyA9YVs5Vs2X5taFFJmyl-5VgrMoj4EIKg0PsNXX-U6WC5INaaAK8fCrltwvj0lM0cRW0CuHSfxyAJZ0HaNph3kBqMCrtTwO5M_XR22RkpTRzolxlli3TxQ
...
echo -e "\n\n\n------------------------------安装 Frp------------------------------"
echo -e "\n\n\n下载 Frp 二进制包"
wget --no-check-certificate -O frp_0.48.0_linux_amd64.tar.gz ${base_url}https://github.com/fatedier/frp/releases/download/v0.48.0/frp_0.48.0_linux_amd64.tar.gz
...
echo -e "\n\n\n 下载并执行 Trojan 快速开始脚本"
bash -c "$(curl -fsSL ${base_url}https://raw.githubusercontent.com/trojan-gfw/trojan-quickstart/master/trojan-quickstart.sh)"
echo -e "\n\n\n 替换配置文件中的证书路径"
...
# 安装可执行文件和 .dat 数据文件
curl -L -s ${base_url}https://raw.githubusercontent.com/v2fly/fhs-install-v2ray/master/install-release.sh | bash

# 只更新 .dat 数据文件
curl -L -s ${base_url}https://raw.githubusercontent.com/v2fly/fhs-install-v2ray/master/install-dat-release.sh | bash
...
# docker exec -t php1 apt-get install -y --no-install-recommends \
# 		libfreetype6-dev \
# 		libicu-dev \
# 		libjpeg-dev \
# 		libmagickwand-dev \
...
# docker exec -t php1 docker-php-ext-configure gd \
# 		--with-freetype \
# 		--with-jpeg \
# 		--with-webp
# # 用 docker-php-ext-install 安装扩展
# docker exec -t php1 docker-php-ext-install -j "$(nproc)" \
# 		bcmath \
# 		exif \
# 		gd \
# 		intl \
